...
FROM(
	SELECT 
		s.externalid as skuseq, 
		to_char(o.orderedat, 'yyyymmdd') as day, 
		d.iyyyy_1_wk AS wk, 
		sum( oi.quantity ) as dailysales
	FROM 
		ods.ORDERS o join ods.ORDER_ITEMS oi on o.id = oi.orderid 
		join ods.skus s on s.id = oi.skuid 
		JOIN bimart.dim_date d ON trunc(d.dates) = to_date(f.rundt,'yyyymmdd' )
	        where o.status <> 'CANCELED'  
		and date(o.orderedat) >= to_date('20170701','yyyymmdd') 
		and skuseq = 3057182
	group by 1,2,3
    ...
    left join sb_scm.scm_daily_forecast f 
    on A.day=f.RUNDT and f.SKU = A.skuseq
where 
    F.RUNDT >= '20170701'
group by 1,2
order by day 
